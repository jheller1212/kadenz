...
--
-- The test in src/db/__tests__/rls-coverage.test.ts asserts on
-- pg_class.relforcerowsecurity, NOT pg_tables.rowsecurity, for exactly this
-- reason: the weaker column is true in both the safe and the unsafe case, so
...
--
-- src/db/__tests__/rls-coverage.test.ts asserts no tenanted column carries a
-- default, so re-adding one to make an insert compile fails the build.
--
--
