...
-- ── push_subscriptions stays GLOBALLY unique on endpoint ─────────────────────
--
-- This is the one table where tenant-scoping the uniqueness would CREATE a leak
-- rather than close one, so it is called out here rather than left unmentioned.
--
-- An earlier draft of this migration rescoped it to (user_id, endpoint), reasoning
-- that a shared household tablet with both athletes signed in produces one
-- endpoint, and that a global constraint lets the second subscriber silently
-- steal the first one's row. The premise is right and the conclusion is
-- backwards.
--
-- A push endpoint addresses ONE browser profile's service worker. Two rows
-- sharing an endpoint do not give two athletes their own subscription; they give
-- one physical device two athletes' reminders. The cron would deliver athlete
-- A's workout title and start time to a notification tray that athlete B is
-- looking at. That is a cross-user disclosure delivered OUTSIDE the app, where
-- row level security cannot reach it, which makes it worse than any in-app read
-- leak: no session, no request, no policy in the path.
--
-- Global uniqueness plus "whoever most recently subscribed owns the endpoint"
-- (the user_id reassignment in saveSubscription, lib/reminders/subscriptions.ts)
-- is therefore the correct behaviour. One device, one owner, the one currently
-- signed in. The previous athlete stops receiving reminders there, which is
-- exactly what should happen when they stop using that browser profile.
--
-- No statement here on purpose: the constraint already exists and must stay.
