...
import { expectsPhysiology, isManualOnly, type DeviceSetup } from "@/lib/device-setup";
...
// Shared with /api/today/bootstrap — see route.ts for the field-by-field
// commentary this was extracted from. `deviceSetup` is passed in rather than
// re-read here because the bootstrap endpoint already loaded it for its own
// device-setup section, and a second read would be the same row twice in one
// request.

export async function getReadinessData(profileId: string | null, deviceSetup: DeviceSetup) {
  const now = Date.now();
  ...
  return { ...result, manualOnly: isManualOnly(deviceSetup) };
}
